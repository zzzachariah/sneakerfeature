...
- [x] **首页 landing + X**:打开 `/` 即 For You(overlay 盖在 slide deck 上,每天首次自动展开、
      X 收起露出浏览、顶部 pill 重开);数据走 `/api/personalize/foryou` 客户端拉取,首页保持缓存。
      （没采用 slides→scroll 重构,改用更轻的 overlay,贴合「输入域名就是 For You + X 关掉」。）
- [ ] 全局转场 / 手势 / 微交互基建
- [ ] 逐屏套用(详情 / 对比 / 智能选鞋 / 导航)
- [ ] 触感设置开关 UI
