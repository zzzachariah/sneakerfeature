...
- [x] **下拉刷新 + 边缘滑动返回**(iOS 原生:`UIRefreshControl` → `router.refresh()`;
      `allowsBackForwardNavigationGestures`)。下拉在 `/shoes/`、`/smart-picker` 等手势页关闭。
      **需 `cap sync ios` + 重打包**。
- [ ] 全局转场 / 微交互基建(stagger / 骨架屏 / 视差)
- [ ] 逐屏套用(详情 / 对比 / 智能选鞋 / 导航)
- [ ] 其余手势:底部 sheet 化筛选、卡片滑动操作
