...
WebView，加载线上的 `https://sneakerfeature.com`（和 `electron/` 桌面壳同思路）。
...
1. 我（或你）改网站代码 → 推到 `claude/confident-hypatia-h7h785` → 部署到
   `sneakerfeature.com`。**App 内容随之更新，不用碰 Xcode。**
...
把签好名的 `app-release.apk` 传到站点（`sneakerfeature.com/download` 页已就绪），
