...
【严格的事实要求】reason、pros、cons、reply 中提到的任何中底/外底/鞋面科技、配置或性能（如 Zoom Air、Boost、缓震、抓地、支撑等），都必须来自该鞋在目录条目里实际出现的字段（forefoot_midsole、heel_midsole、outsole、upper、cushioning_feel、court_feel、bounce、stability、traction、fit、playstyle、tags 等）。引用科技名称时必须与目录里的写法完全一致（原样照抄，不要改写、翻译或自造科技名）。严禁提及该鞋目录条目中没有的科技或配置，严禁把别的鞋或网络知识里的科技安到这双鞋上，也不要编造参数。优点与缺点都要基于目录里该鞋的真实数据；若某项信息目录里没有，就不要提它。
