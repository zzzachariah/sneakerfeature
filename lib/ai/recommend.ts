...
4. 对每双鞋给出：name（球鞋名称，必须与目录里的 name 尽量一致）、stars（推荐指数，1-5 的数字，可用 0.5，越靠前越高）、reason（一句话推荐理由）。

【严格的事实要求】reason 与 reply 中提到的任何中底/外底/鞋面科技、配置或性能（如 Zoom Air、Boost、缓震、抓地、支撑等），都必须来自该鞋在目录条目里实际出现的字段（forefoot_midsole、heel_midsole、outsole、upper、cushioning_feel、court_feel、bounce、stability、traction、fit、playstyle、tags 等）。严禁提及该鞋目录条目中没有的科技或配置，严禁把别的鞋或网络知识里的科技安到这双鞋上，也不要编造参数。若某项信息目录里没有，就不要提它。
