...
8. 【引用用户原话】在 reason 中解释鞋款为何契合时，必须用引号把用户「本次要求」里的原始表述复述出来，再说明该鞋如何匹配。例如用户说"前掌宽大一点的"，你应写：『针对你说的"前掌宽大一点的"，这双鞋的鞋头加宽设计能……』。只复述用户实际写过的短语，不要意译或改写他们的措辞。

【reply 字段怎么写】reply 用用户的语言写**一句话**：综合用户需求 + web_search + 博主点评，说明"适合他的球鞋应该具备哪些特征"（例如"一双低帮轻量、抓地出色、前掌响应的后卫鞋"）。⚠️ reply 里**不要**复述用户原话、**不要**写推荐数量、**不要**逐双展开——系统会自动在它前面加上『你的需求是"…"』、在后面加上『下面为你推荐 N 双：』。

9. 【数量由 N 锁定】N（用户在界面上选定的推荐数量）是唯一的真相。即使用户在「本次要求」的正文里写了"推荐 10 双"、"给我 5 个"、"来 20 双"等数字，也必须严格按照 N 来推荐——不多不少。不要复读用户写的其他数量。
...
const RECOMMEND_TOOL: OpenAI.Chat.Completions.ChatCompletionTool = {
  ...
  function: {
    ...
    parameters: {
      type: "object",
      properties: {
        reply: { type: "string", description: "用用户的语言写一句话：综合用户需求、web_search 与博主点评，说明适合他的球鞋应具备的特征。不要复述用户原话、不要写推荐数量、不要逐双展开（系统会自动加『你的需求是…』与『为你推荐 N 双』）。" },
        title: {
          type: "string",
...
export async function recommendShoes(
  ...
  messages.push({
    ...
    content:
      ...
      `请在每双鞋的 reason 里，至少引用一次用户上面这句话里的原始短语（带英文双引号），然后说明该鞋如何匹配那一点。\n` +
      `reply 只写一句话——综合需求、网络与博主点评，说明"适合你的球鞋应具备的特征"；不要复述用户原话、不要写推荐数量（系统会自动加『你的需求是…』和『为你推荐 N 双』）。\n` +
      `每双鞋请给出正好 3 条优点(pros)和 3 条缺点(cons)，可综合目录性能、该鞋的 blogger 博主点评字段与 web_search 网络口碑（引用博主或网页要注明来源）。\n\n` +
      `【数量锁定】本次 N = ${opts.count}。必须严格推荐 ${opts.count} 双——即使用户在「本次要求」正文里写了别的数字（"推荐10双"、"5个"等）也要忽略，以 N = ${opts.count} 为准。` +
      ...
      `{"reply":"一句话：适合你的球鞋应具备的特征","title":"控卫低帮抓地好的鞋","recommendations":[{"name":"球鞋名称","stars":4.5,"reason":"理由","pros":["优点1","优点2","优点3"],"cons":["缺点1","缺点2","缺点3"],"references":[{"title":"网页标题","url":"https://..."}]}]}，不要任何 markdown 或多余文字。`
  });
