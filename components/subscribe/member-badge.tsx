...
import { skinPalette, memberChipVars, type SkinId } from "@/lib/subscription/skins";
...
export function MemberBadge({ tier, skin, className }: { tier: Tier; skin: SkinId; className?: string }) {
  ...
  return (
    <span
      className={`member-badge-shine member-chip relative inline-flex items-center gap-1 overflow-hidden rounded-full px-2 py-0.5 text-[0.6rem] font-bold uppercase tracking-wide ${className ?? ""}`}
