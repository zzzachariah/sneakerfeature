...
--   * 多重安全闸：时间窗 + outlook 域名 + 非管理员 + 零活动 + 排除你自己的 id。
--     只有"在攻击时间窗内注册、且从没发过任何评论/投稿/收藏/评分/支付"的账号
--     才会被删。真实用户（哪怕也是 outlook 邮箱）只要有任何活动就不会被删。
--     Multiple guards: time window + outlook domain + non-admin + ZERO activity +
--     your own id excluded. A real user is never touched as long as they did
--     anything at all.
--
